...
const {register, handleSubmit, formState:{errors} }= useForm({
    ...
    function LoginUser (data){
        ...
        localStorage.setItem("@kenzieHub:name",
        (res.data.user.name)
        );
        localStorage.setItem("@kenzieHub:course_module",
    ...
    return(
        ...
        <form onSubmit={handleSubmit(LoginUser)} >
            ...
            <label htmlFor="">Email {errors.email && <span className='spanError'> - {errors.email.message}</span>}</label>
            <Input name="email" type="text" placeholder="Digite seu email" register={register} />
            </div>
            <div  className='senha'>

            <label htmlFor="">Senha {errors.password && <span className='spanError'> - {errors.password.message}</span>}</label>
